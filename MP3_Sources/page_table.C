...
void PageTable::handle_fault(REGS * _r)
{
  ...
  if((page_dir[address >> 22] & 1) == 0){
    unsigned long * page_table = (unsigned long *)((kernel_mem_pool->get_frames(1))*PAGE_SIZE);
    page_dir[address >> 22] = ((unsigned long)page_table) | 3;
  }
  unsigned long * pt_adr = ((unsigned long *) (page_dir[address >> 22] & 0xfffff000));
  if(pt_adr[(address & 0x003fffff) >> 12] & 1){
      assert(false);
  }else{
    unsigned long * page = (unsigned long *)((process_mem_pool->get_frames(1))*PAGE_SIZE);
    pt_adr[(address & 0x003fffff) >> 12] = ((unsigned long) page) | mask;
  }
  //assert(false);
